...
describe('plugin-hmr', () => {
    describe('module.hot.accept()', () => {
        ...
        it ('should trigger multiple branch accepts on same level', () => {
            ...
            expect(env.stdout[0]).equal('mod3 dispose');
            expect(env.stdout[1]).equal('mod1 dispose');
            expect(env.stdout[2]).equal('mod2 dispose');
            expect(env.stdout[3]).equal('mod1 accept');
            expect(env.stdout[4]).equal('mod2 accept');
        })
        ...
        it ('should handle circular deps when finding accept', () => {
            ...
            expect(env.stdout[1]).equal('mod2 dispose');
            expect(env.stdout[2]).equal('mod0 accept');
        });
    });
    ...
    describe('module.hot.dispose()', () => {
        it ('should only dispose the module being updated if an accept is there', () => {
            ...
            expect(env.stdout.length).to.equal(2);
            expect(env.stdout[0]).equal('mod0 dispose');
            expect(env.stdout[1]).equal('mod0 accept');
        });
        ...
        it ('should dispose until it finds an accept handler', () => {
            ...
            expect(env.stdout[0]).equal('mod2 dispose');
            expect(env.stdout[1]).equal('mod1 dispose');
            expect(env.stdout[2]).equal('mod0 dispose');
            expect(env.stdout[3]).equal('mod0 accept');
        });
        ...
        it ('should dispose on multiple branches', () => {
            ...
            expect(env.stdout[1]).equal('mod1 dispose');
            expect(env.stdout[2]).equal('mod2 dispose');
            expect(env.stdout[3]).equal('mod0 dispose');
            expect(env.stdout[4]).equal('mod0 accept');
        });
        ...
        it ('should dispose modules, even if accept handler is on a different branch', () => {
            ...
            expect(env.stdout[2]).equal('mod2 dispose');
            expect(env.stdout[3]).equal('mod0 dispose');
            expect(env.stdout[4]).equal('mod1 accept');
        });
    });
    ...
    describe('module.hot.data', () => {
        ...
        it ('should have module.hot.data containing data from original dispose method', () => {
            ...
            env.executeModule(0);
            expect(env.stdout.length).to.equal(1);
            expect(env.stdout[0]).equal('entry {"hello":"world"}');
        });
        ...
        it ('should pass empty object regardless of modifications to hold data into dispose method', () => {
            ...
            expect(env.stdout.length).to.equal(2);
            expect(env.stdout[0]).equal('dispose {}');
            expect(env.stdout[1]).equal('dispose {}');
        });
