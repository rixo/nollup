module.exports = function (options) {
    return {
        nollupBundleInit () {
            return `
                window.__hot_${options.bundleId} = {
                    status: 'idle',
                    options: ${JSON.stringify(options)},
                    statusHandlers: []
                };

                var ws = new WebSocket('ws://' + ${options.hmrHost? `"${options.hmrHost}"` : 'window.location.host'} + '/__hmr_${options.bundleId}');

                function verboseLog() {
                    if (!window.__hot_${options.bundleId}.options.verbose) {
                        return;
                    }
                ...
                function setHotStatus (status) {
                    ...
                    window.__hot_${options.bundleId}.status = status;
                    window.__hot_${options.bundleId}.statusHandlers.forEach(function (handler) {
                        handler(status);
                    });
                }
        ...
        nollupModuleInit () {
            return `
                module.hot = {
                    ...
                    status: function() {
                        return window.__hot_${options.bundleId}.status;
                    },

                    addStatusHandler: function(callback) {
                        window.__hot_${options.bundleId}.statusHandlers.push(callback);
                    },

                    removeStatusHandler: function(callback) {
                        var callbackIndex = window.__hot_${options.bundleId}.statusHandlers.indexOf(callback);
                        if (callbackIndex > -1) {
                            window.__hot_${options.bundleId}.statusHandlers.splice(callbackIndex, 1);
                        }
                    }
                };
            `;
