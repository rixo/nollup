        ...
        try {
            ...
            for (let i = 0; i < bundles.length; i++) {
                ...
                update.output.forEach(obj => {
                    const publicPath = path.relative(options.contentBase, obj.fileName)
                    const inPublic = publicPath.substr(0, 1) !== '.' && !path.isAbsolute(publicPath)
                    const url = posixify(inPublic ? publicPath : obj.fileName)
                    files[url] = obj.isAsset? obj.source : obj.code;
                });
