...
module.exports = function (app, config, options) {
    ...
    async function generateBundles () {
        try {
            if (options.onBundle) options.onBundle({ code: 'BUNDLE_START' })
            ...
            for (let i = 0; i < bundles.length; i++) {
                ...
                const baseUrl = (options.baseUrl || options.contentBase)
                  ? (options.baseUrl || options.contentBase).replace(/^\/+|\/+$/, '') + '/'
                  : null
                ...
                update.output.forEach(obj => {
                    ...
                    if (output.dir) {
                      fileName = path.posix.join(output.dir, obj.fileName)
                    }
                    if (baseUrl && fileName.startsWith(baseUrl)) {
                      fileName = fileName.slice(baseUrl.length)
                    }
            ...
            if (options.onBundle) options.onBundle({ code: 'BUNDLE_END', files })
        } catch (e) {
            console.log('\x1b[91m%s\x1b[0m', e.stack);

            if (options.onBundle) options.onBundle({ code: 'ERROR', error: e })
        }
    }
