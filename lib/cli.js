...
for (let i = 0; i < process.argv.length; i++) {
    ...
    switch (key) {
        ...
        case '--history-api-fallback':
            value = getValue(i);
            if (value) {
                options.historyApiFallback = value;
            } else {
...
(async function () {
    ...
    const index = !options.historyApiFallback
        ? null
        : options.historyApiFallback === true
        ? 'index.html'
    ...
    if (index) {
      app.get(['index.html', '/'], (req, res) => {
        res.sendFile(path.resolve(options.contentBase, index));
      });
    }
    ...
    if (index) {
        app.use(fallback(index, { root: path.resolve(options.contentBase) }));
    }
