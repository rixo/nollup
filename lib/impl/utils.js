...
module.exports = {
    ...
    formatFileName: function (context, fileName, pattern) {
        if (context.chunkNames[fileName]) {
            return context.chunkNames[fileName];
        }

        const base = path.basename(fileName).replace(path.extname(fileName), '');
