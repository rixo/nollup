...
async function generateChunk (context, chunk, bundle, dynamicImports) {
    ...
    if (chunk.isDynamicEntry) {
        ...
        `   var __nollup__import__meta__ = {
            ...
            ${(chunk.exports.length > 0 && isEsm(context.output.format))? 'var __nollup_entry_exports = ' : ''}(function (modules, __nollup__global__) {
            ...
            var __nollup__import__map__add__entry = function (moduleId, chunkFileName) {
                console.log('${path.dirname(chunk.fileName)}', chunkFileName)
        ...
        ${isEsm(context.output.format)? chunk.exports.map(declaration => {
